...
f= open("/nesi/project/uoa02723/Mann_sklearn/Mann_on_training/results_final/sk_grid_lg_0.2_onTrain_saga_c1l1max500_full_modelb_26092019.txt","w+")
...
model_weights.to_csv('/nesi/project/uoa02723/Mann_sklearn/Mann_on_training/results_final/sk_grid_lg_0.2_onTrain_saga_c1l1max500_full_modelb_26092019weights.txt', sep='\t',index=False,line_terminator='\n')
...
parameters = {'C':[1],'max_iter':[500],'l1_ratio':[1]}
lg_clf = LogisticRegression(C=1, max_iter=500, l1_ratio= 1, random_state=1, solver='saga',n_jobs=-1, penalty='elasticnet' )
...
f.write('--------------------------------------------------------------------\n\n')
f.write('No_fold In-Sample AUC: ' + str(model_Max) + '\n')
...
data_array = np.vstack((X_header,best_clf.coef_[0,:]))
model_weights = pd.DataFrame(data=data_array.T,columns=['Data_feature', 'Weight'])
model_weights.to_csv('/nesi/project/uoa02723/Mann_sklearn/Mann_on_training/results_final/sk_grid_lg_0.2_onTrain_saga_c1l1max500_full_modelb_5fold_26092019weights.txt', sep='\t',index=False,line_terminator='\n')
...
data_array = np.vstack((X_header,best_clf.coef_[0,:]))
model_weights = pd.DataFrame(data=data_array.T,columns=['Data_feature', 'Weight'])
model_weights.to_csv('/nesi/project/uoa02723/Mann_sklearn/Mann_on_training/results_final/sk_grid_lg_0.2_onTrain_saga_c1l1max500_full_modelb_nofold_26092019weights.txt', sep='\t',index=False,line_terminator='\n')
