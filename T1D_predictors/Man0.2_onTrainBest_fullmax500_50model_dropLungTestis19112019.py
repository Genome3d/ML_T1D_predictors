...
eQTL_table = pd.read_table('/nesi/project/uoa02723/Mann_sklearn/Mann_on_training/std_Denis_total_Gwas_cat_Denis_2017_all_combined_eQTL_table02042019.txt')
...
#	best_clf = clf_dl
	...
	m_name = '/nesi/project/uoa02723/Mann_sklearn/Mann_on_training/results_50models_LungTestis/sk_grid_lg_0.2_onTrain_50model_dropLung' + str(count) + 'weights.txt'
	model_weights.to_csv(m_name, sep='\t',index=False, line_terminator='\n')
	best_clf = clf_dt
	X_header_dt = np.array(X_train_seldropTestis.columns)
	data_array = np.vstack((X_header_dt,best_clf.coef_[0,:]))
	model_weights = pd.DataFrame(data=data_array.T,columns=['Data_feature', 'Weight'])
	m_name = '/nesi/project/uoa02723/Mann_sklearn/Mann_on_training/results_50models_LungTestis/sk_grid_lg_0.2_onTrain_50model_dropTesis' + str(count) + 'weights.txt'
	model_weights.to_csv(m_name, sep='\t',index=False, line_terminator='\n')
...
# There is no need for step 8.
# 9. Plot distribution of auc_result and compute its mean and standard deviation
# The mean and standard deviation of AUCresult will give you a good overview on the performance of your algorithm.
#AUC_dist=sns.distplot(AUC_result)
#AUC_dist.figure.savefig("/nesi/project/uoa02723/Mann_sklearn/Mann_on_training/results/AUC_results_distribution_c0.7max500l1.png")
AUC_out = pd.DataFrame(AUC_result_dl, columns=['AUC'])
AUC_out.to_csv("/nesi/project/uoa02723/Mann_sklearn/Mann_on_training/results_50models_LungTestis/AUC_results_50modeldropLung.txt", sep='\t',index=False, line_terminator='\n')
AUC_out = pd.DataFrame(AUC_result_dt, columns=['AUC'])
AUC_out.to_csv("/nesi/project/uoa02723/Mann_sklearn/Mann_on_training/results_50models_LungTestis/AUC_results_50modeldropTestis.txt", sep='\t',index=False, line_terminator='\n')
...
f= open("/nesi/project/uoa02723/Mann_sklearn/Mann_on_training/results_50models_LungTestis/Man0.2_onTrianBest_fullmax500_50modelsdropLungTestis.txt","w+")
